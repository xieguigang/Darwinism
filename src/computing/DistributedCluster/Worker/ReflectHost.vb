...
''' 命令行：Worker.exe --mode=worker {blockId} {jobId} {assemblyPath} {methodName} {jobRoot}
''' 职责：
...
Public Class ReflectHost
    ...
    Public Shared Function Run(args As String()) As Integer
        ...
        Catch ex As Exception
            ...
            ' 容错：异常发生在参数解析之前（args 不足）时，回退到当前目录日志。
            Dim blockId = If(args.Length > 1, args(1), "unknown")
            Dim jobRoot = If(args.Length > 5, args(5), ".")
            Try
                File.AppendAllText(
                    Path.Combine(jobRoot, "logs", blockId & ".log"),
                    $"[{DateTime.UtcNow:yyyy-MM-dd HH:mm:ss}] {ex.GetType().Name}: {ex.Message}{vbCrLf}{ex.StackTrace}{vbCrLf}")
            Catch
                ' 日志写入失败也忽略，避免 worker 二次崩溃。
            End Try
